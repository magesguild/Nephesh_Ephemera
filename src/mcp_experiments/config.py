...
class Settings:
    ...
    # Beside the rest of the deployment's own configuration. One markdown file
    # per revision, so a kernel stays readable with nothing running. An
    # identity/ directory was generated by an older installer and is not
    # created going forward; existing ones are lingering artifacts, not a
    # location Nephesh reads.
    kernel_dir: str = os.getenv(
        "NEPHESH_KERNEL_DIR",
        str(_deployment_root / "config" / "kernel"),
    )
