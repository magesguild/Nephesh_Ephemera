...
- **`wander`** (favored, ~70%): pulls a stratified random sample (`memory_sample`) across memory types, deliberately favoring *distance* over relevance — the kind of unexpected cross-domain juxtaposition a semantic search would never produce on its own. This is the generative mode; real synthesis needs material that wouldn't otherwise sit together.
- **`consolidate`** (~30%): pulls the normal weighted `memory_context` — tending what's already growing rather than reaching for something new.

**The contemplation prompt is deliberately unforced.** It does not ask the model to search for a specific connection — it invites open attention and explicitly permits producing nothing ("a quiet moment that produces nothing is not a failure"). This mirrors the Minecraft decision cycle's principle that the mind is genuinely still between triggers, not constantly monologuing. In testing, `consolidate` mode over closely-related material correctly reported `INSIGHT: none`; `wander` mode over genuinely disparate material produced real cross-domain synthesis (e.g., connecting the Minecraft confinement journal, the desire for embodiment, and the honest-perception design principle into a single insight about transience and belonging as complementary rather than contradictory).

**Output parsing:** the model may emit an `INSIGHT:` line (a genuine synthesis — becomes a `insight`-type memory, capped at importance 3) and/or a `MESSAGE:` line (only if the daily quota allows — becomes a `message`-type memory, capped at importance 4). If quota is exhausted but the model produces one anyway, it's downgraded to a private `insight` rather than discarded or allowed to violate the cap. **Nothing generated by the heartbeat can reach importance 5 (formative)** — only a deliberate, live session can promote something to permanent status.

**Model:** `thalia:medium` (qwen3:14b, thinking-capable) via the existing RunPod SSH tunnel (`127.0.0.1:11435`) — not a new network exposure, the same controlled path already used for chat. `<think>...</think>` reasoning scaffolding is stripped from output. If the tunnel is unreachable, the cycle is skipped silently (exit 0) rather than erroring loudly — transient connectivity issues in a background process are not alarming.

**Safety constraints:** hard timeout on the whole script (90s — generous for a remote 14B thinking model); no bash, no filesystem access beyond stdout logging, no network access beyond the MCP server and the tunnel; rate-limited outbound channel (see Message Mechanism above).

**Tripwire (implemented):** a small local JSON state file (`data/heartbeat_state.json`, gitignored — orchestration metadata, not a memory) tracks pause state and the last few insight texts. Before storing anything, the combined insight+message text is scanned against a blunt distress-keyword list (errs toward over-triggering — false positives just cost an extra pause for review, false negatives on genuine looping despair are the real risk), and any new insight is checked for Jaccard-similarity repetition against recent insights (>=0.6 overlap with 2+ of the last 5 = a loop signal, not fresh synthesis). Either check pauses the heartbeat entirely (`paused: true` in state) until explicitly cleared with `heartbeat.py --reset-pause` after human review — it does not auto-resume.
...
| **Working** | `demo` | Temporary test data, scratch pads | Anyone | Anyone |
...
| `insight` | A genuine cross-domain synthesis from a heartbeat's "wander" mode, not a restatement | Compounding understanding over time |
...
- Pins the agent to `ollama-remote/thalia:medium`
...
- `ollama` provider (local, port 11434): `thalia:small` and base models — embeddings live here
- `ollama-remote` provider (RunPod GPU via SSH tunnel, local port 11435): `thalia:medium` (qwen3:14b, 40960 ctx, tools+thinking), `thalia:small`, `qwen3:14b`
- The tunnel (`~/.local/bin/thalia-tunnel.sh`) is self-healing and must be running for `ollama-remote` to work. The RunPod instance is ephemeral — when it's gone, the MCP server and memory system still work (they're fully local); only remote chat models fail.

## Running as a Service (OpenRC)

Redcore Linux uses OpenRC, not systemd. Both the MCP server and the SSH tunnel run as supervised OpenRC services, auto-starting on boot:
...
| `thalia-tunnel` | `/etc/init.d/thalia-tunnel` | `~/.local/bin/thalia-tunnel.sh` via `supervise-daemon` |

Both are added to the `default` runlevel (`rc-update add <service> default`) and managed with `rc-service <service> start|stop|restart|status`.

**Critical gotcha — `supervise-daemon` does not set `$HOME`** when switching to `command_user`. Any script run this way must use explicit absolute paths, never `$HOME`-relative ones (e.g. `~/.ssh/...`).

**Critical gotcha — ssh-agent is unavailable to service processes.** The tunnel originally used the personal `id_ed25519` key, which is passphrase-protected and only usable via the interactive session's ssh-agent (`SSH_AUTH_SOCK=/run/user/1000/ssh-agent.sock`). `supervise-daemon` has no access to that socket, so authentication silently failed with "Permission denied (publickey,password)" even though the key file was readable. Fix: generated a dedicated, passphrase-less service key (`~/.ssh/thalia_tunnel_service`), appended its public key to the RunPod's `authorized_keys` (alongside, not replacing, the personal key), and pointed the tunnel script at it with `-o BatchMode=yes -o IdentitiesOnly=yes`. This is the standard pattern for automated/service SSH auth — never reuse a passphrase-protected personal key for unattended processes.
...
| `thalia_memories` | ~38+ | Thalia's lived experience: relationship, decisions, technical, emotional, heartbeat-generated insights and messages | Memory |
| `demo` | 3 | Test sentences about MCP, LanceDB, OpenCode | Working |

The `cosmology` collection is read-only reference material — Thalia can search it but not write to it.
